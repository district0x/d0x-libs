  ...
  (run-test-async
    (let [txs (subscribe [::tx-subs/txs])
      ...
      (wait-for [::accounts-events/active-account-changed ::accounts-events/accounts-load-failed]
        ...
        (dispatch [::deploy-events/deploy-contract :mintable-token {:from @active-account}])
        (wait-for [::contracts-events/set-contract ::deploy-events/contract-deploy-failed]

          (println "Contract deployed")                     ;; somehow must be here, otherwise chrome disconnects tests
          ...
          (wait-for [::events/add-tx-hash ::tx-events/tx-error]
            (let [tx (first (vals @txs))]
              (is (not (empty? tx)))
              (is (true? @(subscribe [::subs/tx-pending? {:my-tx-id 1}])))

              (wait-for [::tx-events/tx-loaded ::tx-events/tx-error]
                (is (true? @(subscribe [::subs/tx-success? {:my-tx-id 1}])))
                (is (true? @(subscribe [::subs/tx-success? {:my-tx-id 1} {:fn :mint}])))
                (is (false? @(subscribe [::subs/tx-success? {:my-tx-id 1} {:fn :some-other-fn}])))
                (is (false? @(subscribe [::subs/tx-error? {:my-tx-id 1}])))
                (let [tx @(subscribe [::subs/tx {:my-tx-id 1}])]
                  (is (number? (:block-number tx)))
                  (is (= (:transaction-hash tx) @(subscribe [::subs/tx-hash {:my-tx-id 1}])))

                  (dispatch [::tx-events/remove-tx (:transaction-hash tx)])

                  (wait-for [::events/remove-tx-id]
                    (is (empty? @txs))
